...
class CrossModalAlign(CLIPLoss):
    ...
    def diverse_text(self, cores):
        ...
        # _, idxs = torch.topk(torch.abs(distances), 1)
        sampled_edges = torch.rand_like(distances).to(self.args.device)
        # idxs = idxs.item()
        # print(sampled_edges[idxs].shape)
        # sampled_edges[idxs] = 1.0
