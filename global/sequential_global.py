...
def generate_image(s_dict, t, args, style_space, style_names):
    ...
    manip_codes= MSCode(dlatents_loaded, boundary_tmp2, [5.0], device)
...
if __name__=="__main__":
    ...
    parser.add_argument('--trg_lambda', type=float, default=5.0, help="weight for preserving the information of target")
    parser.add_argument('--temperature', type=float, default=3, help="Used for bernoulli")
    ...
    descriptions = ["Receding hairline", "Old", "Eyeglasses"]
    ...
    align_model.text_feature = create_dt("Old woman with receding hairlines wear glasses", align_model.model).to(device)
    align_model.cross_modal_surgery()
    random_text_feature = align_model.diverse_text()
    t, _ = align_model.postprocess(random_text_feature)
    am_img, _ = generate_image(args.s_dict, t, args, style_space, style_names)
    imgs.append(am_img)
