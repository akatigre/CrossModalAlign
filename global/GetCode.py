...
if __name__ == "__main__":
    ...
    # if not os.path.isfile('../pretrained_models/'+dataset_name+'.pkl'):
    #     url='https://nvlabs-fi-cdn.nvidia.com/stylegan2/networks/'
    #     name='stylegan2-'+dataset_name+'-config-f.pkl'
    #     os.system('wget ' +url+name + '  -P  ../pretrained_models/')
    #     os.system('mv ../pretrained_models/'+name+' ../pretrained_models/'+dataset_name+'.pkl')
    ...
    if args.code_type=='w':
        ...
        # tmp = GetCode(Gs,random_state,num_img,num_once,dataset_name)
        w = np.load("./npy/ffhq/W.npy")
        print(w.shape)
        m, std = w.mean(axis=0), w.std(axis=0)
        print(m.shape)
        save_tmp = [m, std]
        with open("./npy/ffhq/W_mean_std", "wb") as fp:
