...
def run_global(args, target, fs3, generator, device):
    ...
    if args.method == "Baseline":
        ...
        exp_name = f'target{target}-chNum{args.topk}-Weight{args.trg_lambda}-lower{args.lb}-upper{args.ub}'
    else:
        exp_name = None
        exit(-1)
    ...
    for i, latent in enumerate(list(subset_latents)):
        ...
        for attmpt in range(args.num_attempts):
            with torch.no_grad():
                unwanted_mask, sc_mask = align_model.disentangle_diverse_text(lb=args.lb, ub=args.ub)
                image_positive_mask = align_model.extract_image_positive()
...
if __name__=="__main__":
    ...
    parser.add_argument('--lb', type=float, default=0.6, help="Threshold for unwanted semantics: elements with cosine similarity larger than lower bound")
    parser.add_argument('--ub', type=float, default=0.4, help="Threshold for unwanted semantics: elements with cosine similarity larger than lower bound")
    ...
    # TODO: Define new target text for afhq
